...
/************************************************/
DROP TABLE IF EXISTS @cdm_results_schema.achilles_result_concept_count;

CREATE TABLE @cdm_results_schema.achilles_result_concept_count
(
  concept_id                int,
  record_count              bigint,
...
WITH counts AS (
  ...
  FROM @cdm_results_schema.achilles_results
  ...
  from (
    ...
    from counts c
    join @cdm_schema.concept_ancestor ca on cast(ca.descendant_concept_id as varchar(50)) = c.concept_id
  ) c
  left join @cdm_schema.concept_ancestor ca on c.concept_id = cast(ca.ancestor_concept_id as varchar(50))
)
INSERT INTO @cdm_results_schema.achilles_result_concept_count (concept_id, record_count, descendant_record_count, person_count, descendant_person_count)
SELECT
